...
class AdminController extends Controller
{
    ...
    public function add_video_process(Request $request) {
        ...
        $video_validator = Validator::make( $request->all(), array(
                    'video'     => 'required|mimes:mkv,mp4,qt,avi',
                    'trailer_video'  => 'required|mimes:mkv,mp4,qt,avi',
                    )
                );
        ...
        if(!$request->hasFile('video') && !$request->hasFile('trailer_video')) {
            return back()->with('flash_error', tr('video_trailer_error').(!$request->hasFile('trailer_video')));
        }
